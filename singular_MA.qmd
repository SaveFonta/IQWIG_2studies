...
# Simply load the pre-calculated object
...
tbl_inputs <- as.data.frame(l$inputs) %>%
  ...
  cols_label(
    ...
    SE       = "Std. Error",
    lower    = "95% CI", # Label changed to reflect the merge below
    width    = "Width"
  ) %>%
  cols_merge(
...
tbl_results <- l$ci %>% 
  gt() %>%
  tab_header(title = "Confidence Interval Estimation Results") %>%
  fmt_number(
...
**NOTE**: the plot and the estimates values for Fixed Effect are reported using the **Inverse Variance** method.
