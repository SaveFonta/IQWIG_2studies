...
Another interesting fact was the way IQWIG reported the raw data. I classified them in 13 possible ways. This also show us which are the most common ways of reporting Meta Anslysis.
...
# Calculate summary statistics stratified by effect.measure and method
width_summary <- widths %>%
...
For a better plotting, I removed the HK 
...
ggplot(widths_noHK, aes(x = method, y = width, fill = method)) +
  ...
  labs(
    ...
    subtitle = "By Effect Measure and Method",
    x = "Method",
    y = "CI Width"
  ) +
  theme_bw() +
...
Reported here:
**Cochran's** $Q$The $Q$-statistic. ($\hat{\theta}_{FE}$).$$Q = \sum_{i=1}^{k} w_i (\hat{\theta}_i - \hat{\theta}_{FE})^2$$where $k$ is the number of studies (here $k=2$), and $w_i = 1/\sigma_i^2$. Under the null hypothesis of homogeneity, $Q$ follows a $\chi^2$ distribution with $k-1$ degrees of freedom.

**Inconsistency Index** ($I^2$)$I^2$ describes the percentage of variation across studies that is due to heterogeneity rather than chance.$$I^2 = \max\left(0, \frac{Q - (k - 1)}{Q}\right) \times 100\%$$
...
This value prevented the graphs do display corretly and detect pattern, so for the next two plots where Q is involved, I decided to remove it.

### Q vs CI Width


```{r}
#| label: fig-q-vs-width-all
#| fig-height: 10
#| fig-width: 10
...
q_all_methods_no_out %>%
  ggplot(aes(x = Q, y = width)) +
  geom_point(alpha = 0.2, size = 1, color = "steelblue") +
  geom_smooth(method = "lm", color = "darkred", se = FALSE) + 
  facet_wrap(~ method, scales = "free_y") + # Free Y because widths differ by method logic
  labs(
    title = "Sensitivity to Heterogeneity: Q vs CI Width",
    x = "Cochran's Q",
    y = "CI Width"
  ) +
  theme_bw() +
...
### Q vs p-values ($H_0: \theta=0$)

```{r}
#| label: fig-q-vs-pvalue-all
#| fig-height: 10
#| fig-width: 10

q_all_methods_no_out %>%
  ggplot(aes(x = Q, y = p_value)) +
  geom_point(alpha = 0.2, size = 1, aes(color = p_value < 0.05)) +
  # Dashed line at 0.05
  ...
  labs(
    title = "Impact of Heterogeneity on p values",
    subtitle = "(Red dots = Significant)",
    x = "Cochran's Q",
    y = "P-value"
  ) +
  theme_bw() +
...
### Summary table
To compute the correlations, I am using the MA with problematic Q-value for completeness 
