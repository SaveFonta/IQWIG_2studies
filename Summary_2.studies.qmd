...
For a better plotting, I removed the HK and also I am not plotting ROM and RD, since they only have respectively 1 and 6 observations.

**Note** that MD and SMD are plotted on the square root scale
...
For a given meta-analysis $x$, let the confidence interval for study $i$ be denoted by $[L_{i,x}, U_{i,x}]$, where $L_{i,x}$ is the lower bound and $U_{i,x}$ is the upper bound. Specifically for the two-study case, we have $L_{1,x}, U_{1,x}$ for study 1 and $L_{2,x}, U_{2,x}$ for study 2.
...
There is a visual bug on the y axis labels, will fix it in the next days 
```{r}
...
summary_hetero <- hetero_df %>% 
  summarise(
    total = n(),
    n_significant_Q = sum(significant_pval),
    prop_significant = mean(significant_pval) * 100,
    avg_I2 = mean(I2),
    avg_tau = mean(Tau2),
    .groups = "drop"
  )
...
summary_hetero %>%
  ...
  cols_label(
    total = "Total MA",
    n_significant_Q = "Significant Q (n)",
    prop_significant = "Significant Q (%)",
    avg_I2 = "Average I2 value",
    avg_tau = "Average tau value"
  ) %>%
  fmt_number(
    columns = c(prop_significant, avg_I2, avg_tau),
    decimals = 2
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
